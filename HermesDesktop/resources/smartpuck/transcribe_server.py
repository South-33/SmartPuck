...
def run_denoising(input_wav_48k: str, output_wav_48k: str):
    ...
    # Limit suppression to preserve quiet consonants and avoid watery speech.
    # DeepFilterNet's unrestricted mode can remove too much room tone on
    # already-clean close-mic recordings.
...
def preprocess_audio(audio_path: str, denoise_mode: str, normalize: bool, denoised_output_path: Optional[str] = None) -> Tuple[str, list]:
    ...
    if normalize or should_denoise:
        if should_denoise:
            ...
            # Give DeepFilterNet a consistent input level first. This is a
            # gentle pre-normalization pass only; final dynamics and target
            # loudness are applied after denoising.
            filter_str = "loudnorm=I=-20:TP=-3:LRA=11" if normalize else "anull"
            cmd = [
            ...
            print(f"[SmartPuck STT] Pre-normalizing & resampling to 48k: {cmd}")
            ...
            # After noise suppression, compress spoken dynamics and apply the
            # final -16 LUFS loudness target used by playback and Whisper.
            ...
            subprocess.run(cmd_16k, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def run_transcription_inner(
    ...
    try:
        ...
        print(
            ...
            f"(profile={profile_key}, model={resolved_model}, lang={language}, denoise={denoise_mode}, normalize={normalize})..."
        )
...
async def transcribe(
    ...
    try:
        ...
        result = run_transcription(temp_path, file.filename, profile_key, language, denoise_mode=denoise_mode, normalize=normalize)
        ...
        if should_reroute_to_khmer(
            ...
            khmer_result = run_transcription(temp_path, file.filename, "khmer-better", "km", denoise_mode=denoise_mode, normalize=normalize)
            ...
            turbo_result = run_transcription(temp_path, file.filename, "high-quality", None, denoise_mode=denoise_mode, normalize=normalize)
...
async def transcribe_local(request: LocalTranscriptionRequest):
