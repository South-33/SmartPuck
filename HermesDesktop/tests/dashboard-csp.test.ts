...
const smartPuckConnectSources = ["http:"];
...
describe("dashboard Content Security Policy", () => {
  ...
  it("allows the renderer to control SmartPuck at its discovered LAN address", () => {
    for (const source of smartPuckConnectSources) {
      expect(mainSrc).toContain(source);
      expect(rendererIndexHtml).toContain(source);
    }
  });
