...
vi.mock("./tools", () => ({ setToolsetEnabled: vi.fn(() => true) }));
...
describe("SmartPuck folder context", () => {
  it("scopes meeting chats to lazy transcript retrieval", () => {
    const message = contextFolderSystemMessage(
      String.raw`D:\Meetings\smartpuck\library\weekly-planning`,
    );
  ...
  it("keeps ordinary folders on the general workspace prompt", () => {
    const message = contextFolderSystemMessage(String.raw`D:\Projects\website`);
    expect(message?.content).toContain("working folder");
    expect(message?.content).not.toContain("meeting assistant");
  });
});
