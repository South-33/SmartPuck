...
import {
  ...
  deleteSmartPuckFolder,
  deleteSmartPuckTranscript,
  deleteSmartPuckRecording,
  importSmartPuckAudioFiles,
  importSmartPuckSessionDirectory,
  listSmartPuckLibrary,
  moveSmartPuckRecording,
  removeSmartPuckRecordingFromFolder,
  readSmartPuckTranscript,
  renameSmartPuckFolder,
  renameSmartPuckRecording,
  saveSmartPuckTranscript,
  smartPuckLibraryRoot,
} from "../smartpuck-library";
import { startSmartPuckTranscription } from "../smartpuck-transcription";
import {
  getSmartPuckDeviceSnapshot,
  controlSmartPuckRecording,
  importSmartPuckDeviceSession,
  deleteSmartPuckDeviceSession,
} from "../smartpuck-device";
import { archiveItem, unarchiveItem, getArchivedItems } from "../archive";
import type {
...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME?.trim() || "SmartPuck";
...
export function registerIpcHandlers(context: IpcContext): void {
  ...
  ipcMain.handle("smartpuck-list-library", () => listSmartPuckLibrary());

  ipcMain.handle("smartpuck-create-folder", (_event, name: string) => {
    if (typeof name !== "string") throw new Error("Folder name is required.");
    return createSmartPuckFolder(name);
  });

  ipcMain.handle(
    "smartpuck-rename-folder",
    (_event, folderId: string, name: string) =>
      renameSmartPuckFolder(folderId, name),
  );

  ipcMain.handle(
    "smartpuck-delete-folder",
    (_event, folderId: string) =>
      deleteSmartPuckFolder(folderId),
  );

  ipcMain.handle(
    "smartpuck-archive-item",
    (_event, itemId: string, itemType: "folder" | "session") =>
      archiveItem(itemId, itemType),
  );

  ipcMain.handle(
    "smartpuck-unarchive-item",
    (_event, itemId: string) =>
      unarchiveItem(itemId),
  );

  ipcMain.handle(
    "smartpuck-list-archived-items",
    () => getArchivedItems(),
  );

  ipcMain.on(
    "smartpuck-show-folder-context-menu",
    (event, folderId: string, folderName: string, folderPath: string) => {
      const win = BrowserWindow.fromWebContents(event.sender);
      if (!win) return;
      const template: Electron.MenuItemConstructorOptions[] = [
        {
          ...
          click: () => {
            win.webContents.send("smartpuck-new-folder-chat-requested", folderPath, folderName);
          },
        },
        { type: "separator" },
        {
          label: "Rename folder...",
          click: () => {
            win.webContents.send("smartpuck-rename-folder-requested", folderId, folderName);
          },
        },
        {
          label: "Archive folder",
          click: () => {
            win.webContents.send("smartpuck-archive-folder-requested", folderId, folderName);
          },
        },
      ];
      Menu.buildFromTemplate(template).popup({ window: win });
  ...
  ipcMain.on(
    "smartpuck-show-session-context-menu",
    (event, sessionId: string, sessionTitle: string) => {
      const win = BrowserWindow.fromWebContents(event.sender);
      if (!win) return;
      const template: Electron.MenuItemConstructorOptions[] = [
        {
          label: "Rename chat...",
          click: () => {
            win.webContents.send("smartpuck-rename-session-requested", sessionId, sessionTitle);
          },
        },
        {
          label: "Archive chat",
          click: () => {
            win.webContents.send("smartpuck-archive-session-requested", sessionId, sessionTitle);
          },
        },
      ];
      Menu.buildFromTemplate(template).popup({ window: win });
  ...
  ipcMain.handle("smartpuck-read-transcript", (_event, recordingId: string) =>
    readSmartPuckTranscript(recordingId),
  );

  ipcMain.handle(
    "smartpuck-save-transcript",
    (_event, recordingId: string, content: string) =>
      saveSmartPuckTranscript(recordingId, content),
  );

  ipcMain.handle("smartpuck-delete-transcript", (_event, recordingId: string) =>
    deleteSmartPuckTranscript(recordingId),
  );

  ipcMain.handle("smartpuck-delete-recording", (_event, recordingId: string) =>
    deleteSmartPuckRecording(recordingId),
  );

  ipcMain.handle(
    "smartpuck-rename-recording",
    (_event, recordingId: string, title: string) =>
      renameSmartPuckRecording(recordingId, title),
  );

  ipcMain.handle("smartpuck-select-audio-files", async (event) => {
    const win = BrowserWindow.fromWebContents(event.sender);
  ...
  ipcMain.handle(
    "smartpuck-import-audio-files",
    (_event, folderId: string | null, filePaths: string[]) => {
      const safeFolderId =
      ...
      const safeFilePaths = Array.isArray(filePaths)
        ...
        : [];
      return importSmartPuckAudioFiles(safeFolderId, safeFilePaths);
    },
  );

  ipcMain.handle(
    "smartpuck-select-and-import-session-folder",
    async (event, folderId: string | null) => {
      const win = BrowserWindow.fromWebContents(event.sender);
      const result = win
        ? await dialog.showOpenDialog(win, { properties: ["openDirectory"] })
        : await dialog.showOpenDialog({ properties: ["openDirectory"] });
      ...
      const safeFolderId =
        typeof folderId === "string" && folderId.trim() ? folderId : null;
      return importSmartPuckSessionDirectory(safeFolderId, result.filePaths[0]);
    },
  );

  ipcMain.handle("smartpuck-open-path", async (_event, target: string) => {
    const pathToOpen =
  ...
  ipcMain.handle(
    ...
    ) => startSmartPuckTranscription(recordingId, modelProfile, language, denoiseMode, normalizeAudio),
  );

  ipcMain.handle("smartpuck-device-snapshot", (_event, deviceUrl: string) =>
    getSmartPuckDeviceSnapshot(deviceUrl),
  );

  ipcMain.handle(
    "smartpuck-control-recording",
    (_event, deviceUrl: string, action: "start" | "stop") =>
      controlSmartPuckRecording(deviceUrl, action),
  );
  ...
  ipcMain.handle(
    ...
    ) => importSmartPuckDeviceSession(folderId, deviceUrl, session),
  );

  ipcMain.handle(
    "smartpuck-move-recording",
    (_event, recordingId: string, targetFolderId: string) =>
      moveSmartPuckRecording(recordingId, targetFolderId),
  );
  ipcMain.handle(
    "smartpuck-remove-recording-from-folder",
    (_event, recordingId: string, folderId: string) =>
      removeSmartPuckRecordingFromFolder(recordingId, folderId),
  );

  ipcMain.handle(
    "smartpuck-delete-device-session",
    (_event, deviceUrl: string, sessionPath: string) =>
      deleteSmartPuckDeviceSession(deviceUrl, sessionPath),
  );
