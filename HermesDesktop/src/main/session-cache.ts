...
import { getArchivedSessionIds } from "./archive";
...
export function syncSessionCache(): CachedSession[] {
  ...
  try {
    ...
    return updated.sessions.filter((s) => !archivedSessionIds.has(s.id));
  } catch (err) {
    const archivedSessionIds = getArchivedSessionIds();
    return cache.sessions.filter((s) => !archivedSessionIds.has(s.id));
  }
}
...
export function listCachedSessions(limit = 50, offset = 0): CachedSession[] {
  const cache = readCache();
  const archivedSessionIds = getArchivedSessionIds();
  const filtered = cache.sessions.filter((s) => !archivedSessionIds.has(s.id));
