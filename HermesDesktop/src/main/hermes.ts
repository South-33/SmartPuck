...
export function contextFolderSystemMessage(
  ...
  const isSmartPuckFolder = /[\\/]smartpuck[\\/]library(?:[\\/]|$)/i.test(
    folder,
  );
  if (isSmartPuckFolder) {
    return {
      role: "system",
      content:
        `You are SmartPuck, a focused local meeting assistant. The active ` +
        `meeting collection is ${folder}. Answer from meeting recordings and ` +
        `transcripts, not from the operating system or development environment. ` +
        `Do not mention Hermes, agents, toolsets, terminals, processes, or ` +
        `internal implementation unless the user explicitly asks about them. ` +
        `Use file search/read only when meeting evidence is needed; never list ` +
        `processes or inspect unrelated system state. Be direct for casual ` +
        `questions. For meeting questions, cite timestamps when available, ` +
        `state uncertainty clearly, and never invent decisions or action items. ` +
        `Search sibling collections only when the user asks to compare meetings.` ,
    };
  }
  return {
    role: "system",
...
function sendMessageViaCli(
  ...
  const env: Record<string, string> = {
    ...
    // SmartPuck supplies its own narrow meeting system prompt and selected
    // folder context. Do not also inject Hermes' bundled developer AGENTS.md
    // into every meeting turn; it consumes most of the request budget and is
    // unrelated to recordings, transcripts, or folders.
...
export function buildGatewayEnv(profile?: string): Record<string, string> {
  ...
  const profileDir = profileHome(resolveProfile(profile));
  // Hermes bridges terminal.cwd into TERMINAL_CWD at gateway startup, which
  // overrides any process env value. Keep the daemon away from its bundled
  // source checkout (and its large AGENTS.md); meeting sessions still provide
  // their own directory to the file tools.
  if (getConfigValue("terminal.cwd", profile) !== profileDir) {
    setConfigValue("terminal.cwd", profileDir, profile);
  }
  // SmartPuck is a local meeting assistant. Limit the agent to the tools it
  // needs to inspect its selected workspace instead of exposing Hermes' broad
  // skills catalogue and unrelated web/productivity workflows.
  setToolsetEnabled("hermes-cli", false, profile);
  setToolsetEnabled("skills", false, profile);
  setToolsetEnabled("file", true, profile);
  setToolsetEnabled("terminal", false, profile);
  const gatewayEnv: Record<string, string> = {
    ...
    HERMES_HOME: HERMES_HOME,
    HERMES_IGNORE_RULES: "1",
    // The upstream agent resolves automatic AGENTS.md context from this
    // variable. Anchor it outside the bundled checkout; each SmartPuck chat
    // still supplies its selected meeting folder to the file-tool session.
    TERMINAL_CWD: profileDir,
...
export function startGatewayDetailed(profile?: string): GatewayStartResult {
  ...
  try {
    proc = spawn(HERMES_PYTHON, hermesCliArgs(cliArgs), {
      // Do not launch the user-facing gateway from the bundled agent checkout:
      // its large development AGENTS.md is otherwise injected into every
      // session before the meeting workspace instructions. The profile home is
      // stable, user-owned, and leaves each selected meeting as the tool cwd.
      cwd: profileHome(resolveProfile(profile)),
      env: gatewayEnv,
