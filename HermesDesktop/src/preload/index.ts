...
const hermesAPI = {
  ...
  smartPuck: {
    listLibrary: (): Promise<SmartPuckLibrarySnapshot> =>
      ipcRenderer.invoke("smartpuck-list-library"),
    createFolder: (name: string): Promise<SmartPuckFolder> =>
      ipcRenderer.invoke("smartpuck-create-folder", name),
    renameFolder: (folderId: string, name: string): Promise<SmartPuckFolder> =>
      ipcRenderer.invoke("smartpuck-rename-folder", folderId, name),
    readTranscript: (recordingId: string): Promise<string> =>
      ipcRenderer.invoke("smartpuck-read-transcript", recordingId),
    saveTranscript: (
      recordingId: string,
      content: string,
    ): Promise<SmartPuckRecording> =>
      ipcRenderer.invoke("smartpuck-save-transcript", recordingId, content),
    deleteTranscript: (recordingId: string): Promise<SmartPuckRecording> =>
      ipcRenderer.invoke("smartpuck-delete-transcript", recordingId),
    deleteRecording: (recordingId: string): Promise<void> =>
      ipcRenderer.invoke("smartpuck-delete-recording", recordingId),
    renameRecording: (
      recordingId: string,
      title: string,
    ): Promise<SmartPuckRecording> =>
      ipcRenderer.invoke("smartpuck-rename-recording", recordingId, title),
    selectAudioFiles: (): Promise<string[]> =>
      ipcRenderer.invoke("smartpuck-select-audio-files"),
    importAudioFiles: (
      folderId: string | null,
      filePaths: string[],
    ): Promise<SmartPuckImportResult> =>
      ipcRenderer.invoke("smartpuck-import-audio-files", folderId, filePaths),
    selectAndImportSessionFolder: (
      folderId: string | null,
    ): Promise<SmartPuckImportResult | null> =>
      ipcRenderer.invoke(
        "smartpuck-select-and-import-session-folder",
        folderId,
      ),
    openPath: (
      target?: string,
    ): Promise<{ success: boolean; error?: string }> =>
      ipcRenderer.invoke("smartpuck-open-path", target ?? ""),
    queueTranscription: (
      recordingId: string,
    ...
    getDeviceSnapshot: (deviceUrl: string): Promise<SmartPuckDeviceSnapshot> =>
      ipcRenderer.invoke("smartpuck-device-snapshot", deviceUrl),
    controlRecording: (
      deviceUrl: string,
      action: "start" | "stop",
    ): Promise<void> =>
      ipcRenderer.invoke("smartpuck-control-recording", deviceUrl, action),
    importDeviceSession: (
      folderId: string | null,
      deviceUrl: string,
      ...
      ipcRenderer.invoke(
        "smartpuck-import-device-session",
        folderId,
        deviceUrl,
        session,
      ),
    moveRecording: (
      ...
      ipcRenderer.invoke("smartpuck-move-recording", recordingId, targetFolderId),
    removeRecordingFromFolder: (
      recordingId: string,
      folderId: string,
    ): Promise<void> =>
      ipcRenderer.invoke(
        "smartpuck-remove-recording-from-folder",
        recordingId,
        folderId,
      ),
    deleteDeviceSession: (
      ...
      ipcRenderer.invoke("smartpuck-delete-device-session", deviceUrl, sessionPath),
    deleteFolder: (folderId: string): Promise<void> =>
      ipcRenderer.invoke("smartpuck-delete-folder", folderId),
    archiveItem: (itemId: string, itemType: "folder" | "session"): Promise<void> =>
      ipcRenderer.invoke("smartpuck-archive-item", itemId, itemType),
    unarchiveItem: (itemId: string): Promise<void> =>
      ipcRenderer.invoke("smartpuck-unarchive-item", itemId),
    listArchivedItems: (): Promise<Array<{ itemId: string; itemType: string; archivedAt: number }>> =>
      ipcRenderer.invoke("smartpuck-list-archived-items"),
    showFolderContextMenu: (folderId: string, folderName: string, folderPath: string): void => {
      ipcRenderer.send("smartpuck-show-folder-context-menu", folderId, folderName, folderPath);
    },
    showSessionContextMenu: (sessionId: string, sessionTitle: string): void => {
      ipcRenderer.send("smartpuck-show-session-context-menu", sessionId, sessionTitle);
    },
    onRenameFolderRequested: (callback: (folderId: string, folderName: string) => void): (() => void) => {
      const handler = (_event: any, folderId: string, folderName: string) => callback(folderId, folderName);
      ipcRenderer.on("smartpuck-rename-folder-requested", handler);
      return () => {
        ipcRenderer.removeListener("smartpuck-rename-folder-requested", handler);
      };
    },
    onArchiveFolderRequested: (callback: (folderId: string, folderName: string) => void): (() => void) => {
      const handler = (_event: any, folderId: string, folderName: string) => callback(folderId, folderName);
      ipcRenderer.on("smartpuck-archive-folder-requested", handler);
      return () => {
        ipcRenderer.removeListener("smartpuck-archive-folder-requested", handler);
      };
    },
    onNewFolderChatRequested: (callback: (folderPath: string, folderName: string) => void): (() => void) => {
      const handler = (_event: any, folderPath: string, folderName: string) => callback(folderPath, folderName);
      ipcRenderer.on("smartpuck-new-folder-chat-requested", handler);
      return () => {
        ipcRenderer.removeListener("smartpuck-new-folder-chat-requested", handler);
      };
    },
    onRenameSessionRequested: (callback: (sessionId: string, sessionTitle: string) => void): (() => void) => {
      const handler = (_event: any, sessionId: string, sessionTitle: string) => callback(sessionId, sessionTitle);
      ipcRenderer.on("smartpuck-rename-session-requested", handler);
      return () => {
        ipcRenderer.removeListener("smartpuck-rename-session-requested", handler);
      };
    },
    onArchiveSessionRequested: (callback: (sessionId: string, sessionTitle: string) => void): (() => void) => {
      const handler = (_event: any, sessionId: string, sessionTitle: string) => callback(sessionId, sessionTitle);
      ipcRenderer.on("smartpuck-archive-session-requested", handler);
      return () => {
        ipcRenderer.removeListener("smartpuck-archive-session-requested", handler);
      };
    },
  },
