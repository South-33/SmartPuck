...
export interface ChatRun {
  ...
  seed?: ChatMessage[];
  /** Optional folder to bind before the first turn, used by SmartPuck meetings. */
...
export function isScratchRun(r: ChatRun): boolean {
  return !r.sessionId && !r.loading && !r.title && !r.initialContextFolder;
}
...
export function mintRun(
  ...
  if (initialContextFolder) run.initialContextFolder = initialContextFolder;
  return run;
}
