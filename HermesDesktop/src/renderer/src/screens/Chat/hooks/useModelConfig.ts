...
const SMARTPUCK_PROVIDER = "openai-codex";
const SMARTPUCK_DEFAULT_MODEL = "gpt-5.4-mini";
...
export function effectiveOverrideBaseUrl(
  ...
  return provider === "custom" || provider === "ollama-cloud"
    ? baseUrl
    : "";
}
...
export function useModelConfig(profile?: string): UseModelConfigResult {
  ...
  const codexDiscovery = useDiscoveredModels({
    provider: SMARTPUCK_PROVIDER,
    profile,
    enabled: true,
  });

  const modelsForPicker = useMemo(() => {
    const discovered = codexDiscovery.models.map((model) => ({
      provider: SMARTPUCK_PROVIDER,
      model,
      name: model,
      baseUrl: "",
    ...
    return discovered.length > 0
      ? discovered
      : savedModels.filter((model) => model.provider === SMARTPUCK_PROVIDER);
  }, [savedModels, codexDiscovery.models]);
  ...
  const reload = useCallback(async (): Promise<void> => {
    ...
    setCurrentModel(mc.model || SMARTPUCK_DEFAULT_MODEL);
    setCurrentProvider(
      mc.provider === SMARTPUCK_PROVIDER ? mc.provider : SMARTPUCK_PROVIDER,
    );
