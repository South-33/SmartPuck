...
- Speaker diarization is automatic only for shorter recordings; long recordings skip it by default because it is CPU-heavy and can produce noisy speaker counts. The ONNX diarizer uses up to `SMARTPUCK_DIARIZATION_THREADS` threads, defaulting to a conservative CPU-aware value. Transcript speaker cleanup can still be a normal Markdown edit.
