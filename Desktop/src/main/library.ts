...
- **ASR Evidence sections**: \`## ASR Evidence\` contains full-pass English and Khmer reference transcripts. Because they use parallel VAD-clip timestamps matching the main transcript, you can easily align segments line-by-line using the timestamp brackets (e.g. \`[00:00:02]\`). Use these reference passes to repair missing/garbled transcript spans, translate phonetically misheard words back to the correct language (e.g. when the English model writes gibberish for a Khmer segment), correct spelling, and then remove the evidence section from the final curated output.
