...
- **ASR Evidence sections**: \`## ASR Evidence\` contains parallel English and Khmer reference transcripts. Align them line-by-line using timestamps. 
  - **Be conservative:** Only replace a segment if the main transcript is obvious gibberish (e.g. "Yung Thua Chow") and the reference pass has a clear, grammatically correct phrase in the other language. 
  - **Do NOT guess:** If the main transcript is a clear, meaningful sentence (e.g. "We're drowning") and the reference is a weak phonetic guess, keep the original main text. 
  - **Preserve Evidence if unsure:** If there is any doubt or mixed-language ambiguity, leave the \`## ASR Evidence\` section intact at the bottom of the file so the user can verify it. Only delete the evidence section if you are 100% certain of a perfect, clean merge.
